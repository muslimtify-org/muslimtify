...
int main(void) {
  ...
  check_action(trigger_catchup_action(590, 600), TRIGGER_FIRE, "missed 10 min ago fires (within window)");
  check_action(trigger_catchup_action(585, 600), TRIGGER_FIRE, "missed exactly 15 min ago fires (window edge)");
  check_action(trigger_catchup_action(584, 600), TRIGGER_DROP, "missed 16 min ago dropped (past window)");
